...
function _addTrashIcon(li, trace, application) {
  ...
  trash.onclick = function () {
    (0, _util.deleteApplicationTraces)([trace.uuid], trace.app_id).then(function (json) {
...
function _updateTabBadge(application, traceUuid) {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    chrome.runtime.sendMessage({
      action: _util.DELETE_TRACE,
      tab: tab,
      application: application,
      traceUuid: traceUuid
    });

    chrome.browserAction.getBadgeText({ tabId: tab.id }, function (currentBadgeText) {
      console.log("CURRENT BADGE TEXT", currentBadgeText);
      var currentBadgeValue = parseInt(currentBadgeText, 10);
      ...
      (0, _util.updateTabBadge)(tab, (currentBadgeValue - 1).toString(), _util.CONTRAST_RED);
    });
  });
}
