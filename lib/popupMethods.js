...
function getStorageVulnsAndRender(items, application) {
  ...
  chrome.runtime.sendMessage({ action: _util.TRACES_REQUEST, application: application }, function (response) {
    if (response && response.traces && response.traces.length > 0) {
      ...
      populateVulnerabilitySection(response.traces, items[_util.TEAMSERVER_URL], items[_util.CONTRAST_ORG_UUID]);
    } else {
