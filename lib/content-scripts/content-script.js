...
function _initializeContrast(request, sendResponse) {
  ...
  var apiKey = _dataQuery(0);
  var orgUuid = _dataQuery(1);
  // const teamServerUrl = _dataQuery(2);
  var serviceKey = _dataQuery(3);
  var profileEmail = _dataQuery(4);
  ...
  chrome.storage.local.set(contrastObj, function () {
    ...
    sendResponse({ action: CONTRAST_INITIALIZED, contrastObj: contrastObj });
  });
}
