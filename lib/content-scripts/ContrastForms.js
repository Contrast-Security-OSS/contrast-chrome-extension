...
ContrastForm.collectFormActions = function (sendResponse) {
  ...
  chrome.storage.local.get(STORED_APPS_KEY, function (result) {
    ...
    if (window.CONTRAST__REFRESHED === false) {
      ...
      if (!!actions) {
        messageSent = true;
        _this.__sendFormActionsToBackground(actions, sendResponse);
        return;
      }
    }
    ...
    if (!messageSent) {
      var obs = new MutationObserver(function (mutations, observer) {
        _this._collectMutatedForms(mutations, observer, sendResponse);
      });
...
ContrastForm._getFormsFromMutations = function (mutations) {
  ...
  return mutations.map(function (mutation) {
    ...
    if (!!mutatedForms && mutatedForms.length > 0) {
      var form = new ContrastForm(mutatedForms);
      var actions = form._extractActionsFromForm();
