...
ConnectedDomain.prototype._addDomainToStorage = function () {
  ...
  return new _promise2.default(function (resolve, reject) {
    chrome.storage.local.get(_util.STORED_APPS_KEY, function (result) {
      ...
      var verified = _this._verifyDomainNotInUse(result[_util.STORED_APPS_KEY], host);
...
ConnectedDomain.prototype._verifyDomainNotInUse = function (storedApps, host) {
  if (storedApps.length > 0) {
    for (var i = 0, len = storedApps.length; i < len; i++) {
      ...
      if (app.domain === host) {
        return new Error('The Domain ' + _Application2.default._subColonOrUnderscore(host) + ' is already in use by another application: ' + app.name + '. Please either first disconnect ' + app.name + ' or run this application on a different domain/port.');
      }
    }
  }
  return true;
