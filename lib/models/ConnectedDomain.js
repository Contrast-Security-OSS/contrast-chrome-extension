...
ConnectedDomain.prototype._addDomainToStorage = function () {
  ...
  return new _promise2.default(function (resolve, reject) {
    chrome.storage.local.get(_util.STORED_APPS_KEY, function (result) {
      ...
      if (result[_util.STORED_APPS_KEY].length > 0) {
        for (var i = 0, len = result[_util.STORED_APPS_KEY].length; i < len; i++) {
          var _app = result[_util.STORED_APPS_KEY][i];
          if (_app.domain === host) {
            return reject(new Error('The Domain ' + host + ' is already in use by another application: ' + _app.name + '. Please either first disconnect that application or run this application on a different domain/port.'));
          }
        }
      }
...
ConnectedDomain.prototype._removeDomainFromStorage = function () {
  ...
  return new _promise2.default(function (resolve, reject) {
    ...
    chrome.storage.local.get(_util.STORED_APPS_KEY, function (result) {
      var updatedStoredApps = _this._filterOutApp(result);
...
ConnectedDomain.prototype._filterOutApp = function (storedApps) {
  ...
  return storedApps[_util.STORED_APPS_KEY].filter(function (app) {
    return app.id !== _this2.application.id;
  });
};