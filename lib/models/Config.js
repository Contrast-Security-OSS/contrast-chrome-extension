...
Config.prototype.renderContrastUsername = function () {
  ...
  (0, _util.setElementText)(userEmail, this.credentials[_util.CONTRAST_USERNAME]);
  userEmail.addEventListener("click", this._addListenerToUsername, false);
};

Config.prototype._addListenerToUsername = function () {
  var contrastIndex = this.credentials[_util.TEAMSERVER_URL].indexOf("/api");
  var teamserverUrl = this.credentials[_util.TEAMSERVER_URL].substring(0, contrastIndex);
...
Config.prototype._setBackButton = function (set, callback) {
  var username = document.getElementById('user-email');
  username.removeEventListener('click', this._addListenerToUsername);
  username.removeEventListener('click', callback);
  if (set) {
    (0, _util.setElementText)(username, "Back");
    username.addEventListener('click', callback);
  } else {
    (0, _util.setElementText)(username, this.credentials[_util.CONTRAST_USERNAME]);
