    ...
    chrome.storage.local.get(_util.STORED_APPS_KEY, function (result) {
      ...
      if (!application) {
        ...
        if (application && application.name) {
          var appLibText = document.getElementById('scan-libs-text');
          if (appLibText) {
...
Application.getStoredApp = function (appsInStorage, application) {
  ...
  if ((0, _util.isEmptyObject)(appsInStorage)) return;
  var storedApps = appsInStorage[_util.STORED_APPS_KEY] || appsInStorage;
  return storedApps.filter(function (app) {
