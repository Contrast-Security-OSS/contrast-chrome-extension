...
var DELETE_TRACE = exports.DELETE_TRACE = "contrast__remove_storedTrace";
...
function deleteApplicationTraces(traces, appId) {
  return getStoredCredentials().then(function (items) {
    var url = deleteApplicationTracesUrl(items[TEAMSERVER_URL], items[CONTRAST_ORG_UUID], appId);
    var authHeader = getAuthorizationHeader(items[CONTRAST_USERNAME], items[CONTRAST_SERVICE_KEY]);

    return fetchTeamserver(url, { traces: traces }, authHeader, items[CONTRAST_API_KEY], 'DELETE');
  });
}
...
exports.deleteApplicationTraces = deleteApplicationTraces;
