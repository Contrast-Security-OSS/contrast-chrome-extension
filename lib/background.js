...
chrome.webRequest.onBeforeRequest.addListener(function (request) {
	...
	if (request.type === "xmlhttprequest" && !(0, _util.isBlacklisted)(request.url)) {
		chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
			var tab = tabs[0];
			if (!tab || tab.url.includes(_util.TEAMSERVER_API_PATH_SUFFIX)) return;
...
function handleRuntimeOnMessage(request, sendResponse, tab) {
	if (request === _util.TRACES_REQUEST) {
		...
		(0, _util.getStoredCredentials)().then(function (creds) {
			var formActions = request.formActions;

			if (!!formActions) {
				evaluateVulnerabilities((0, _util.isCredentialed)(creds), tab, formActions);
			}
		});
	}
}
...
function setToStorage(foundTraces, tab) {
	...
	var traces = foundTraces.filter(function (t) {
		return !!t;
	});

	buildVulnerabilitiesArray(traces, tab).then(function (vulnerabilities) {
		var storedTraces = {};
		storedTraces[_util.STORED_TRACES_KEY] = JSON.stringify(vulnerabilities);
