...
var _handleRuntimeOnMessage = function () {
	var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(request, sendResponse, tab) {
		var tabPath, vulnerableTab, storedTabs, application, traceUuid, path, vulnTab, storedTab, storedTabTraces, filteredTraces;
		return _regenerator2.default.wrap(function _callee$(_context) {
			while (1) {
				switch (_context.prev = _context.next) {
					case 0:
						_context.t0 = request.action;
						_context.next = _context.t0 === _util.TRACES_REQUEST ? 3 : _context.t0 === _util.APPLICATION_CONNECTED ? 12 : _context.t0 === _util.APPLICATION_DISCONNECTED ? 14 : _context.t0 === _util.LOADING_DONE ? 16 : _context.t0 === _util.DELETE_TRACE ? 18 : 29;
						break;
					...
					case 18:
						application = request.application, traceUuid = request.traceUuid;
						path = _VulnerableTab2.default.buildTabPath(tab.url);
						vulnTab = new _VulnerableTab2.default(path, application.name);
						_context.next = 23;
						return vulnTab.getStoredTab();

					case 23:
						storedTab = _context.sent;
						storedTabTraces = storedTab[vulnTab.vulnTabId];
						filteredTraces = storedTabTraces.filter(function (t) {
							return t !== traceUuid;
						});
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	...
	if (request.action !== _util.TRACES_REQUEST && request.action !== _util.LOADING_DONE && request.action !== _util.DELETE_TRACE) {
		if (!TAB_CLOSED) {
