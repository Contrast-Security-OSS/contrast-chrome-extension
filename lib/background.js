...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	chrome.tabs.query({ active: true }, function (tabs) {
		...
		if (request !== _util.TRACES_REQUEST && request !== _util.EVALUATE_XHR) {
			if (!TAB_CLOSED) {
	...
	if (request === _util.TRACES_REQUEST) {
		chrome.storage.local.get(_util.STORED_TRACES_KEY, function (result) {
			if (!!result && !!result[_util.STORED_TRACES_KEY]) {
				sendResponse({ traces: result[_util.STORED_TRACES_KEY] });
			} else {
				sendResponse({ traces: [] });
			}
			(0, _util.removeLoadingBadge)(tab);
		});
	} else if (request === _util.EVALUATE_XHR && CURRENT_APPLICATION) {
